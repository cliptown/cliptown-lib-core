...
func Validate(value any) error { if err := validate.Struct(value); err != nil { return fmt.Errorf("public validation failed: %w", err) }; return nil }
func DecodeAndValidate[T any](data []byte) (T, error) { var value T; decoder := json.NewDecoder(bytes.NewReader(data)); decoder.DisallowUnknownFields(); if err := decoder.Decode(&value); err != nil { return value, fmt.Errorf("decode public value: %w", err) }; if err := decoder.Decode(&struct{}{}); err != io.EOF { if err == nil { return value, fmt.Errorf("decode public value: trailing JSON value") }; return value, fmt.Errorf("decode public value: %w", err) }; if err := Validate(value); err != nil { return value, err }; return value, nil }
