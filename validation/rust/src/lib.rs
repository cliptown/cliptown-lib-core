...
mod tests {
    ...
    fn preserves_exact_identifier_text() {
        let result = serde_json::from_str::<RequestMeta>(
            r#"{"requestId":" req-1 ","traceId":" trace-1 "}"#,
        );
        assert!(result.is_ok(), "valid request metadata must decode");
