...
test("keys carry the org prefix", () => {
  assert.equal(key("jobs", "x"), "cliptown/jobs/x");
  assert.equal(ORG, "cliptown");
});

test("placeholders are filled in order and every entry plans", () => {
  assert.equal(entryKey({ domain: "jobs", name: "{a}/x/{b}", layers: { fiducia: true, pgAdvisory: true }, pgScope: "transaction", wait: true }, "1", "2"), "cliptown/jobs/1/x/2");
