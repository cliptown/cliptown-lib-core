...
fn every_supported_model_boundary_fits_canonical_storage() {
    ...
    for (provider, model, dimensions) in cases {
        ...
        assert_eq!(
            embedding.values().get(dimensions.saturating_sub(1)).copied(),
            Some(1.0)
        );
        assert!(embedding
            .values()
...
fn padding_preserves_prefix_order_sign_and_norm() {
    ...
    assert!(result.is_ok(), "two-dimensional MRL output must be accepted");
    let Ok(embedding) = result else {
        return;
    };
...
fn model_profiles_are_monotonic_and_never_exceed_source_capacity() {
    ...
    for (provider, model) in models {
        ...
        assert!(profile.is_some(), "registered model must retain a dimension profile");
